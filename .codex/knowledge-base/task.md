...
```
Organizer or same user? Simplest: only organizer sees jobs for their event_id; or only
internal/debug. Pick one rule and document.
```
...
```
```
API inserts a row in the same DB transaction as the registration;
worker polls WHERE status = 'pending'.
```
...
```
API publishes message; worker consumes.
```
